	...
	<div id="sidebar" align="center">
		...
		<c:choose>
			<c:when test="${not empty principal}">
				<img id="userProfileImage" src="/upload/${principal.user.profile_image_url}" onerror="this.src='/image/profile.jpg'" class="rounded-circle profile" onclick="profileImageUpload(${principal.user.id})">
	...
	<div id="content">
		<form action="/" method="GET" class="form-inline p-2 bd-highlight justify-content-between">
			<div class="dropdown" style="float: left">
				...
				<div class="dropdown-menu">
					<a class="dropdown-item" href="/?page=${param.page}&sort=id,DESC&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}">최신순</a> <a class="dropdown-item"
						href="/?page=${param.page}&sort=count,DESC&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}">인기순</a> <a class="dropdown-item"
						href="/?page=${param.page}&sort=recommendCount,DESC&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}">추천순</a>
				</div>
			</div>
		...
		<table class="table table-hover">
			...
			<c:forEach var="board" items="${boards.content}">
				...
				<tbody style="<c:if test='${fn:contains(board.seen, user)}'>color: gray;</c:if>">
					<tr onclick="location.href='/board/${board.id}/?page=${param.page}&sort=${param.sort}&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}&sortType=${param.sortType}'"
		...
		<ul class="pagination justify-content-center">
			<li class="page-item <c:if test='${boards.number < 5}'>disabled</c:if>"><a class="page-link"
				href="/?page=${startPage - 5}&sort=${param.sort}&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}"><</a></li>
			<c:forEach var="page" begin="1" end="5">
				<c:if test="${(startPage + page) <= boards.totalPages}">
					<li class="page-item <c:if test='${boards.number eq startPage + page - 1}'>active</c:if>"><a class="page-link"
						href="/?page=${startPage + page - 1}&sort=${param.sort}&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}">${startPage + page}</a></li>
				</c:if>
			</c:forEach>
